...
// -------------------------------------------------------------------
// vkFFT 1D Plan, Execute, and Cleanup Functions
// -------------------------------------------------------------------
...
WrFFTErrors vkfft1d_plan(int n, Vkfft1DContext* context, WrFFTPrecision precision) {
    ...
    configuration.size[0] = n; //FFT size
    switch(precision){
    ...
    context->buffer_size = elem_bytes(precision) * 2 * n;
    ...
    // Allocate Memory on GPU
        ...
        resFFT = allocateBuffer(&context->gpu, &buffer, &bufferDeviceMemory, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_HEAP_DEVICE_LOCAL_BIT, &context->buffer_size);
...
// #else
//     resFFT = transferDataToCPU(&context->gpu, host_data, &context->device_buffer, context->buffer_size);
//     std::cout << "Result transferred to CPU" << std::endl;
//     if (resFFT != VKFFT_SUCCESS) {
//         delete context;
//         return WRFFT_ERROR_LIBRARY_FAILURE;
//     }
